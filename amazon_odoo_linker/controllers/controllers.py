...
from ...amazon_odoo_linker import utils as amazon_utils
...
class AmazonInfoController(http.Controller):
    ...
    def amazon_return_from_authorization(self, **data):
        ...
        account = request.env['amazon.info.account'].browse(account_id).exists()
        if not account:
        ...
        try:
            ...
            return request.render(
                'amazon_odoo_linker.authorization_error',
    ...
    def _compute_account_url(account_id):
        ...
        :rtype: str
        """
        action = request.env.ref('amazon_odoo_linker.list_amazon_account_action', raise_if_not_found=False)
